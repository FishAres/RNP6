...
args = Dict(
    ...
    :f_z => identity, # state RNN activation - identity to stay Gaussian
...
args[:imzprod] = prod(args[:img_size]) # img size product
...
# Compiling the backward pass will take a couple minutes
@info "Training..."
losses = train_model(ps, train_loader, test_loader, save_dir; n_epochs=150, save_every=50, logger=lg, clip_grads=true, plotting_func=imview_color)