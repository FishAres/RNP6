...
curve_primitive = let
    ...
    curve_resized = imresize(reshape(curve_primitive, 28, 28, :), (6, 6))[:, :, 1]
    curve_conv = repeat(curve_resized, 1, 1, 1, 1)
    curve_conv
end

const dec_filters = cat(line_primitive, curve_primitive, dims=4) |> gpu
...
const dec_cdims = let
    ...
    m = ConvTranspose((6, 6), 2 => 1, stride=2)
...
Decoder = Chain(
    ...
    BasicBlock(64 => 64, +),
    BasicBlock(64 => 64, +),
    ConvTranspose((4, 4), 64 => 2, stride=1, pad=1),
    BatchNorm(2, elu),
...
## =====
save_folder = "two_primitive_comparison"
